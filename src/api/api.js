...
export default {
    getApartments: (data) => {
        ...
        return http.get(`/apartments/?location__region=${!region ? '' : region}&location__city=${!city ? '' : city}&location__district=${''}&type=${!construction_type ? '' : construction_type}&room=${!rooms ? '' : rooms}&floor=${!floor ? '' : floor}&construction_type=${!apartmentType ? '' : apartmentType}&state=${''}&min_price=${!priceFrom ? '' : priceFrom}&max_price=${!priceTo ? '' : priceTo}&currency=${''}&arrival_date=${fromDate}&departure_date=${toDate}&min_area=${''}&max_area=${''}&rental_period=${''}&objects_in_apartment=${!nearbyObjects ? '' : nearbyObjects}&nearby_objects=${!objects_in_apartment ? '' : objects_in_apartment}`)
